...
public:
    zstring() = default;
    explicit zstring(decltype(nullptr)) = delete;
    explicit zstring(const char* s) { assign(s, strlen(s)); }
    zstring(size_t n_ch, char ch = '\0') { assign(n_ch, ch); }
    ...
    void assign(const char* s, size_t n_ch) {
        if (n_ch > get_sso_capacity()) heap_assign(s, n_ch);
        else sso_assign(s, n_ch);
    }

    void assign(size_t n_ch, char ch) {
        if (n_ch > get_sso_capacity()) heap_assign(n_ch, ch);
        else sso_assign(n_ch, ch);
    }

    void reserve(size_t n_ch) {
        // if enough, do nothing
        // so if using OSS and n_byte <= oss_capacity, never do anything
        if (n_ch <= capacity()) return;
        else realloc_buffer(n_ch + 1);
    }
...
private:
    void realloc_buffer(size_t n_byte_capa) {
        ...
        if (!is_on_heap()) {
            ...
            if (n_byte_capa > get_sso_capacity()) {
                ...
                size_t new_heap_capa_byte = to_aligned_size(n_byte_capa);
                char* new_buf_ptr = (char*)malloc(new_heap_capa_byte);
                memcpy(new_buf_ptr, get_sso_buf_const(), old_sso_size);
                new_buf_ptr[old_sso_size + 1] = '\0';

                // start writing storage, original data on SSO will be broken
                set_heap_mode();
                set_heap_size(old_sso_size);
                set_heap_capacity(new_heap_capa_byte);
            ...
            if (n_byte_capa > get_heap_capacity()) {
                ...
                size_t new_heap_capa_byte = to_aligned_size(n_byte_capa);
                char* new_buf_ptr = (char*)malloc(new_heap_capa_byte);
                memcpy(new_buf_ptr, old_heap_ptr, old_heap_size);
                new_buf_ptr[old_heap_size + 1] = '\0';

                free(old_heap_ptr);
                set_heap_ptr(new_buf_ptr);
                set_heap_capacity(new_heap_capa_byte);
                set_heap_size(old_heap_size);
            } else if (n_byte_capa < get_heap_capacity()) {
                char* old_heap_ptr = get_heap_ptr();

                size_t new_heap_capa_byte = to_aligned_size(n_byte_capa);
                char* new_buf_ptr = (char*)malloc(new_heap_capa_byte);
                memcpy(new_buf_ptr, old_heap_ptr, new_heap_capa_byte);
                new_buf_ptr[new_heap_capa_byte] = '\0';

                free(old_heap_ptr);
                set_heap_ptr(new_buf_ptr);
                set_heap_capacity(new_heap_capa_byte);
                set_heap_size(new_heap_capa_byte - 1);
            }
            // if n_byte_capa = heap_size, do nothing
        }
    }

    void sso_assign(const char* s, size_t n_ch) {
        assert(n_ch <= get_sso_capacity());
        if (is_on_heap()) free_heap_ptr_sized_dirty();
        set_sso_mode();
        set_sso_size(n_ch);
        set_sso_buf(s, n_ch);
    ...
    void sso_assign(size_t n_ch, char ch) {
        assert(n_ch <= get_sso_capacity());
        if (is_on_heap()) free_heap_ptr_sized_dirty();
        set_sso_mode();
        set_sso_size(n_ch);
    ...
    void heap_assign(const char* s, size_t n_ch) {
        // if current data on heap, free first
        if (is_on_heap()) free_heap_ptr_sized_dirty();
        size_t heap_capa_byte = to_aligned_size(n_ch + 1);
        char* heap_ptr = (char*)malloc(heap_capa_byte);
        memcpy(heap_ptr, s, n_ch);
        heap_ptr[n_ch + 1] = '\0';
        set_heap_mode();
        set_heap_capacity(heap_capa_byte);
        set_heap_ptr(heap_ptr);
        set_heap_size(n_ch);
    }

    void heap_assign(size_t n_ch, char ch) {
        // if current data on heap, free first
        if (is_on_heap()) free_heap_ptr_sized_dirty();
        size_t heap_capa_byte = to_aligned_size(n_ch + 1);
        char* heap_ptr = (char*)malloc(heap_capa_byte);
        memset(heap_ptr, ch, n_ch);
        heap_ptr[n_ch + 1] = '\0';
        set_heap_mode();
        set_heap_capacity(heap_capa_byte);
        set_heap_ptr(heap_ptr);
...
private:
    ...
    void set_sso_size(size_t n_ch) { storage.sso.set_size(n_ch); }
    void set_sso_buf(const char* s, size_t n_ch) { storage.sso.set_buf(s, n_ch); }
    void set_sso_buf(size_t n_ch, char ch) { storage.sso.set_buf(n_ch, ch); }
    ...
    void set_heap_size(size_t n_ch) { storage.heap.set_size(n_ch); }
...
public:
    constexpr static size_t sso_capacity_byte = 23;

    class sso_storage {
        unsigned char size_;           // available character count, exclude \0
        char buf_[sso_capacity_byte];  // store as byte count, include \0

    public:
        size_t size() const { return size_ >> 1; }
        size_t capacity() const { return sso_capacity_byte - 1; }
        char* buf() { return buf_; }
        const char* buf_const() const { return buf_; }

        void set_size(size_t n_ch) {
            assert(n_ch <= capacity());
            size_ &= 1;
            size_ |= n_ch << 1;
        }
        void set_buf(const char* s, size_t n_ch) {
            assert(n_ch <= capacity());
            memcpy(buf_, s, n_ch);
            buf_[n_ch + 1] = '\0';
        }
        void set_buf(size_t n_ch, char ch) {
            assert(n_ch <= capacity());
            memset(buf_, ch, n_ch);
            buf_[n_ch + 1] = '\0';
        }
    ...
    class heap_storage {
        size_t capacity_;  // store as byte count, include \0
        size_t size_;      // available character count, exclude \0
        char* ptr_;

    public:
        size_t capacity() const { return ((capacity_ >> 1) << 1) - 1; }
        size_t size() const { return size_; }
        char* ptr() { return ptr_; }
        const char* ptr_const() const { return ptr_; }

        void set_capacity(size_t n_byte) {
            assert((n_byte & 1) == 0);
            capacity_ &= 1;
            capacity_ |= n_byte;
        }
        void set_size(size_t n_ch) { size_ = n_ch; }
